...
export class Menu extends Phaser.GameObjects.Container {
  ...
  addButton(text, callback) {
    const buttonY = this.title
      ? this.title.y + 100 + this.buttons.length * this.options.spacing
      : this.buttons.length * this.options.spacing;
  ...
  repositionButtons() {
    this.buttons.forEach((button, index) => {
      const buttonY = this.title
        ? this.title.y + 100 + index * this.options.spacing
        : index * this.options.spacing;
      button.y = buttonY;
    });
  }
